...
{
	salary = 7000.0 + skill / 100.0 * 3500.0;
}
...
void Coach::Inc_skill(Player *_player[])
{
		for (int i = 0; i < 11; ++i)
		{
			if ((double)rand() / RAND_MAX < skill / 100.0)
			{
				_player[i]->skill += skill * 0.015;
				if (_player[i]->skill > 100.0)
				{
...
{
	salary = 6000.0 + skill / 100.0 * 3000.0;
}
...
void Physio::Inc_health(Player *_player[])
{
	for (int i = 0; i < 11; ++i)
	{
		_player[i]->health += skill / 10.0;
		if (_player[i]->health > 100.0)
...
{
	salary = 5000.0 + skill / 100.0 * 2500.0;
}
...
void Game_result(Team &_t1, Team &_t2, string &_output_file)
{
	...
	double t1_pass_ball_probablity = (_t1.attack_ability - _t2.defend_ability) * 0.3 / 100.0 ;
	double t2_pass_ball_probablity = (_t2.attack_ability - _t1.defend_ability) * 0.3 / 100.0 ;
	if (t1_pass_ball_probablity < 0.3)
	{
	...
	for (int i = 0; i < 100; ++i)
	{
		if (_t1.Ball_possession && count_pass < 6)
		{
		...
		if (_t1.Ball_possession && count_pass == 6)
		{
			int num = rand() % (_t1.num_attacker + _t1.num_midfielder);
			double shot_success_probablity = (_t1.player[num]->skill * (_t1.player[num]->health / 100.0) - _t2.player[10]->skill * (_t2.player[10]->health / 100.0)) / 100.0;
	...
	_t1.Ball_possession = false;
	_t2.Ball_possession = false;

	for (int j = 0; j < 11; ++j)
	{
		_t1.player[j]->Dec_health();
		_t2.player[j]->Dec_health();
	}

	_t1.goal += t1_goal;
	_t2.goal += t2_goal;
	_t1.per_goal.push_back(t1_goal);
...
void Game_play(Team &_t1, Team &_t2, int &n, string &_output_file)
{
	...
	for (int i = 0; i < n;)
	{
		...
		if (gameday_flag)
		{
			...
			Game_result(_t1, _t2, _output_file);
			Display_player(_t1, _output_file);
			Display_player(_t2, _output_file);
			cout << _t1.tname << " " << _t1.per_goal[i] << " VS " << _t2.per_goal[i] << " " << _t2.tname << endl;
			fout << _t1.tname << " " << _t1.per_goal[i] << " VS " << _t2.per_goal[i] << " " << _t2.tname << endl;
			_t1.staff[2]->count_spend(_t1, _output_file);
		...
		{
			_t1.staff[0]->Inc_skill(_t1.player);
			_t2.staff[0]->Inc_skill(_t2.player);
			_t1.staff[1]->Inc_health(_t1.player);
